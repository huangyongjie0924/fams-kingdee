...
> **实施备注（2026-09-20 落地）**：最终未新增 `IsCategoryRepairable`；`handleCreateRepair` 直接读 `GetCard` 带出的 `card.CategoryRepairable`（`cardSelect` 的 `COALESCE(cat.repairable,0)`），语义等价且省一次查库。设计段保留，以记录「当初为什么考虑这个函数」。
...
- **返回结构**（一次请求返回「待办 + 指标卡 + 概览」三块，前端同一份结构渲染）：

```json
{
  "todo": [
    { "key": "pending",    "label": "待受理", "count": 3, "link": "/repairs?status=pending",  "level": "danger" },
    { "key": "unassigned", "label": "待派工", "count": 1, "link": "/repairs?status=accepted", "level": "warning" }
  ],
  "stats": [
    { "key": "asset_total", "label": "资产总数", "count": 204, "link": "/assets" },
...
> `level` 是紧急程度的语义色（`danger`/`warning`/`info`），同样由服务端给：前端只映射到配色，不按 key 猜。

#### 各角色「与我相关」指标卡（`stats`，P0 范围）

与 `todo` 的分工：**todo = 要你去做的事（count=0 不出）**，**stats = 与你相关的量（该角色适用就出，0 也出）**。
设计宗旨要求「第一屏内容 = 与我相关的事」，只有 `todo` + 全量 `overview` 时，员工首页只能拿全量概览冒充——`stats` 补的正是这块。

| 角色 | 指标卡 | 数据源 / 收窄口径 |
|---|---|---|
| admin / asset_manager | 资产总数 / 待受理 / 待派工 / 待确认 | `asset_card`（复用 `overview.asset_total`，不重复查库）+ `repair_order`（管理视角，全局） |
| counter / dept_head / repair_tech / viewer | 我的资产 / 我的报修 / 我的维修 / 我的待盘点 | `asset_card` 按 `user_emp_id=me`、`repair_order` 按 `reporter_emp_id=me` 与 `assignee_emp_id=me`（且排除终态 `done/rejected/cancelled`）、`count_item` 按 `assignee_id=me` |

> 员工视角四个计数**一律按归属人收窄**，`empID<=0` 返回 `0` 而**不退化为全量**——否则就回到「员工看到管理员首页」。
> `link` 一律指向 `web/src/router/index.ts` 中真实存在的路由：`/assets`、`/repairs`、`/count/mine`。
...
**可忽略。** 首页一个请求发 3–4 条聚合查询（资产状态分布、资产总数、维修单状态、待办计数），加 `stats` 后员工视角再 +4 条按归属人收窄的 `COUNT(*)`（管理者视角的「资产总数」复用 `overview`，不重复查库），合计 ≤ 8 条。在 **227 行**量级下，每条全表聚合在**亚毫秒**返回，单个请求占用连接的时长极短。相较资产列表（带多表 JOIN + 分页 + COUNT）占用更少。
...
10. **`cardSelect` 的 `cat` 是 LEFT JOIN**：任何取自 `cat` 的列都要 `COALESCE`——**防御性必要**（LEFT JOIN 未匹配或 `category_id=0` 哨兵值时 `cat.repairable` 为 NULL，`Scan` 进 `bool` 会报错）。当前生产库无 `category_id=0` 数据，故非「必然」，但代码必须对该情形免疫。
