...
> **FAMS**（Fixed Asset Management System）—— 与星瀚（金蝶）ERP 联动的集团固定资产台账系统。
> 组织主数据（公司 / 部门 / 员工）与资产卡由星瀚 ERP 托管、每日凌晨定时同步；
> 台账侧负责卡片维护、财务信息批量更新、二维码标签与盘点执行。

> 项目背景、业务框架导图与全过程优化点见 **[项目介绍](docs/项目介绍.md)**
> （图文版：[项目介绍.html](docs/项目介绍.html)）。

> **关于本仓库**：组织主数据里的公司名称与员工姓名已做化名处理（`XXX公司`、`张三` 等）；
> `fi.kingdee.com` / `yunzhijia.com` 为**示例地址**，实际部署需替换成你自己的星瀚与云之家实例。
> 组织编码（`GCGS` / `12` / `1201` / `1202`）与接口路径保持原样，便于对照星瀚返回结构。

> 项目级设计纲领（设计宗旨、设计原则、评审检查清单）见 **[设计宗旨](docs/设计宗旨.md)**。

## 技术栈

- 后端：Go 1.25，`database/sql` + stdlib `net/http`，无 ORM 无 Web 框架
- 前端：Vue 3 + Element Plus + Vite，产物用 `go:embed` 打进二进制
- 数据库：MySQL 8.0，库名 `asset`，20 张表启动时自动建
...
> **星瀚内部同名不算冲突，也不需要处理。** `1201` 和 `1202` 是两个法人，
> 各自有「财务部」「市场部」等同名部门是**上游真实存在的组织事实**，
> 它们编码不同（如 `120102` / `120214`）、各挂各的公司，会落成两行并列存在。
> 同名冲突检查只针对「本地手工行 vs 本次同步行」，不会把这种正常情况报出来。
> 实测 88 个部门里有 24 组这样的同名，全部 `source = 'kingdee'`。

> **`1202` 子树的部门在台账里会显示 0 员工，这不是漏同步。**
> 多数范围内人员在两个法人下各挂一条同一职位的记录（2026-09-18 快照：
> 136 人里 118 人「1201 + 1202」双挂、18 人只在 1201、0 人只在 1202）。
> 台账 `employee.dept_id` 是单值，按 `pickPrimaryDept` 的规则**固定优先取 1201**，
> 所以那 118 人的主职都落在 1201。全部候选部门如实保留在 `OrgEmployee.DeptCodes` 里。
> 只有「在 1202 有任职、且 1201 下没有任何候选」的人才会落到 1202（有测试守着）。

> **为什么 dry-run 要在运行记录里标出来**：dry-run 也把「会新建 / 会更新」记进统计列
> （这样历史里能看到影响面的变化趋势），但这会让演练和真写在记录里长得一样。
> 所以演练批次的摘要固定写「演练模式（dry_run）：未写库，新增/更新为预估数」。

### 重建组织主数据（`cmd/orgreset`）

历史遗留的部门/员工行是「资产卡同步」早期按名称兜底建出来的：只有编码和名称，
`parent_id` / `company_id` / `longnumber` 全空。要把它们换成星瀚的完整数据，
用 `orgreset`：

```bash
# 默认 dry-run：预检 + 备份 + 打印将要做什么，不写库
go run ./cmd/orgreset -config config.yaml

# 真跑：必须同时给出与配置一致的库名
go run ./cmd/orgreset -config config.yaml -dry-run=false -confirm=asset
```

五步：**预检 → 备份 → 清理 → 组织同步 → 资产卡同步**，中间夹一步账号重绑，最后打体检报告。

**⚠ 必须连 `external_master_map` 一起清，这不是可选项。** 映射表存的是
「星瀚编码 → 本地行 ID」。只删三张表、不清映射的话，组织同步查映射会拿到
**已经被删掉的 ID**，当成"已存在的行"去 UPDATE——UPDATE 影响 0 行、也不报错，
那些行就再也不会被创建出来。库副本上实测（删表但不清映射）：

| | 不删直接同步 | **删表但不清映射** | 删表 + 清映射 + 两步同步 |
|---|---|---|---|
| 公司 | 3 | **0** | 2 |
| 部门 | 79（10 组塌陷） | **59 / 88** | **88** |
| 员工 | 136 | **71 / 136** | 136 |
| 父节点孤儿 | 0 | **32** | 0 |
| 归属公司孤儿 | 0 | **59（全部）** | 0 |
| 使用部门断链 | 0 | **202 / 202** | 2 |
...
顺序也是固定的：**组织主数据 → 资产卡**。组织同步只写主数据表、不碰 `asset_card`，
卡片上的 `use_dept_id` / `user_emp_id` / `owner_company_id` 全靠资产卡同步按编码重新解析写回，
这是引用能接上的唯一途径。

三道护栏：

- **删之前先预检**：只读拉一次星瀚，范围内部门或人员为 0 就直接中止
  （一次接口故障不该把主数据删干净而重建不出来）。
- **备份先落盘**：`external_master_map` + 三张主数据表导出成可回灌的 INSERT 语句，
  写不成功就不往下走。实测回灌能精确还原清理前的行数与内容。
- **库名确认**：真跑要求 `-confirm` 与配置里的库名完全一致，防止连错库。

> dry-run 承诺「不写库」，所以**它不留下备份文件**：试写一次、量出字节数、删掉。
> 既验证了「备份这一步不会失败」（这正是 dry-run 该回答的问题），
> 又不会让反复 dry-run 堆出一堆 100KB 的垃圾、或让人误以为已经备份过了。

体检报告只统计**界面上看得见的卡**（`deleted_at IS NULL`），软删除的卡单独报一行。
两条口径都不能省：

- 把软删除的算进断链 → 报出一批用户找不到、也不需要处理的卡，**影响面虚高**；
- 把 `0` 当成悬空引用 → `use_dept_id = 0` 是「没填」不是「断了」，**虚报假警报**。

所有外键统计都带 `> 0` 下界（`0` 是本地约定的"未设置"哨兵值，不是外键值）。

**账号绑定会按工号自动重绑。** 员工行被删掉重建、ID 全变，`sys_user.employee_id`
因此断链——这是工具自己造成的回归，不该留给人工收尾。按 `emp_no` 匹配而不是按姓名：
工号在人员接口里是唯一键，姓名会重名。匹配不上就保持原样并报出来，不猜——
把 A 的账号绑到 B 头上比断链严重得多。对照（账号 → 工号）在清理前抓取，
并写进备份文件末尾作为 `UPDATE` 语句，进程中途被杀也能手工恢复。

体检报告会列出引用接不上的卡片。工具**不会替你猜**归属——星瀚里没有的部门/公司
（比如本地自建的 `SJ XXX公司`）和不在同步范围的人员，需要人工指定。
